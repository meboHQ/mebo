...
describe('Web Middleware Auth:', () => {
  ...
  before((done) => {
    ...
    Mebo.addBeforeAuthAction(passportAuth);
  ...
  after(() => {
    ...
    Mebo.Ext.Handlers.Web._beforeAuthActionMiddlewares = [];
  ...
  it('Should fail to perform an action that requires auth without any login', (done) => {
    Mebo.webfyAction(testutils.Actions.Shared.Sum, 'get', {auth: true, rest: false});

    app.get('/authSum', Mebo.middleware('sum', (err, result, req, res, next) => {
