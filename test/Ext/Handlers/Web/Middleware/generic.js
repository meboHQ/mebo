...
describe('Web Middleware Generic:', () => {
  ...
  before((done) => {
    ...
    Mebo.registerAction(TestBeforeMiddleware);
  ...
  after(() => {
    ...
    Mebo.Ext.Handlers.Web._beforeActionMiddlewares = [];
  ...
  it('Should perform an action', (done) => {
    Mebo.webfyAction(testutils.Actions.Shared.Sum, 'get');

    app.get('/sum', Mebo.middleware('sum', (err, result, req, res, next) => {
  ...
  it('Should fail to perform an action through a non webfied method (patch)', (done) => {
    Mebo.webfyAction(testutils.Actions.Shared.Sum, 'get', {auth: false, rest: false});

    app.patch('/sum', Mebo.middleware('sum', (err, result, req, res, next) => {
      if (err) return next(err);
      res.send(`result: ${result}`);
    }));
  ...
  it('Should perform an action and handle the error result', (done) => {
    ...
    app.get('/sum2', Mebo.middleware('sum', (err, result, req, res, next) => {
  ...
  it('Tests if Mebo.addBeforeAction middlewares are being called', (done) => {
    ...
    Mebo.webfyAction(TestBeforeMiddleware, 'get');

    app.get('/beforeMiddlewareTest', Mebo.middleware('TestBeforeMiddleware', (err, result, req, res, next) => {
      if (err) return next(err);
      res.send(`result: ${result}`);
    }));
